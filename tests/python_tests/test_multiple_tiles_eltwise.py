...
def test_multiple_tiles(testname, formats, dest_acc, mathop, math_fidelity, tile_cnt):
    if (
        ...
        and formats.get_input_format() == DataFormat.Float16
    ...
    src_A, src_B = generate_stimuli(
        formats.get_input_format(), formats.get_input_format(), tile_cnt=tile_cnt
    )  # , const_face=True, const_value_A=3, const_value_B=2)
    golden = generate_golden(mathop, src_A, src_B, formats.get_output_format(), math_fidelity)
    write_stimuli_to_l1(
        src_A, src_B, formats.get_input_format(), formats.get_input_format(), "0,0", tile_cnt
    )
